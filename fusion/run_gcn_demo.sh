...
if [ $MODE == "GCNSingleLayerCompare" ]; then
  ...
  sr=1
  while read line; do
#    for BCOL in {8,1000,3000}; do
      for ED in {32,64,128}; do
        for tn in {16,32,64,128,256,512,1024,2048,4096,8192}; do
#          for mw in {4,6,8,10,12,14,16}; do
            echo "for $line $BCOL $ED $tn $mw"
            if [ $header -eq 1 ]; then
              $BINPATH/gcn_layer_demo -sm $DATA/$line -nt $THREADS -tn $tn -ah -sr $sr -bc $ED -ed $ED -en $MODE > ./build/logs/gcn_single_layer_demo.csv
              header=0
            else
              $BINPATH/gcn_layer_demo -sm $DATA/$line -nt $THREADS -tn $tn -sr $sr -bc $ED -ed $ED -en $MODE >> ./build/logs/gcn_single_layer_demo.csv
            fi
#          done
        done
#      done
