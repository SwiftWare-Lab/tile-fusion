...
#include "../sparse-tiling/sparse-tiling.h"
...
protected:
  ...
  Timer analysis() override {
   ...
   auto k2 = new bool[A_csc_->m]();
   for (int i = 0; i < sched.size(); ++i) {
    for (int j = 0; j < sched[i].size(); ++j) {
     for (int k = 0; k < sched[i][j].size(); ++k) {
      std::cout<<sched[i][j][k].id<<" "<<sched[i][j][k].kernel_id<<",";
      assert(sched[i][j][k].id == vertices[cc]);
      assert(sched[i][j][k].kernel_id == vert_types[cc]);
      cc++;
      if(!sched[i][j][k].kernel_id){
       assert(!k1[sched[i][j][k].id]);
       k1[sched[i][j][k].id] = true;
      }
      else{
       assert(!k2[sched[i][j][k].id]);
       k2[sched[i][j][k].id] = true;
      }
     }
     std::cout<<";";
    }
...
//    auto *tmp = new double[num_threads_*n_]();
    auto *ws = new double[InTensor->NumThreads * FusedCompSet->n1_]();
    OutTensor->reset();
    Timer t;
    t.start();
...
//    sym_lib::spmv_csr_spmv_csr_sparse_tiling(n_,A_csr_->p,A_csr_->i,
//                                     A_csr_->x,x_in_,y,x_,
//                                     levels,
//                                     spar_ptr,
//                                     wpar_ptr,
...
public:
  ...
  SpmvSpmvSparseTiling(TensorInputs<double> *In1, Stats *Stat1,
                sym_lib::ScheduleParameters SpIn)
      : SpMVSpMVFused(In1, Stat1, SpIn) {
//    L1_csc_ = L_csc; L1_csr_ = L;
//    A_csr_ = A; A_csc_ = A_csc;
