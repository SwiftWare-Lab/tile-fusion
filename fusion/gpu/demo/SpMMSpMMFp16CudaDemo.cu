...
//
#include "../Cuda_SpMM_SpMM_fp16_Demo_Utils.h"
...
int main (const int argc, const char *argv[]) {
  ...
  delete inSpMMTemp;
  auto *inSpMM =
      new CudaTensorInputsFP16(aCSCFull->m, tp._b_cols, aCSCFull->m, aCSCFull->m,
                               aCSCFull, aCSCFull, numThread, numTrial, expName);
...
//  delete stats;
  ...
  for (auto ts: tileSizes){
    ...
    auto *unfusedSeqReduceRowCoarsened = new SpMMSpMMSeqReduceRowBalanceCoarsenedRowFP16(inSpMM,stats, ts);
