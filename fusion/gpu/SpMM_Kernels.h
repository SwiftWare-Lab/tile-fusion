...
void csrspmm_parreduce_rowbalance(const int M, const int N, const int K, const int* Ap,
                                  const int* Ai, const float* Ax, const float *B,
                                  float *C) {
...
void csrspmm_parreduce_nnzbalance(const int M, const int N, const int K, const int NNZ, const int* Ap,
                                  const int* Ai, const float* Ax, const float *B,
                                  float *C) {
    // factor of thread coarsening
    int coarsen_factor = (N % 4 == 0) ? 4 : (N % 2 == 0) ? 2 : 1;
...
void csrspmm_rowcaching_rowbalance(const int M, const int N, const int K, const int* Ap,
                                  const int* Ai, const float* Ax, const float *B,
                                  float *C) {
...
void csrspmm_rowcaching_nnzbalance(const int M, const int N, const int K, const int NNZ,
    ...
    if (coarsen_factor == 4) {
      ...
      csrspmm_rowcaching_nnzbalance_kernel<4, 1>
          <<<gridDim, blockDim, smem_size>>>(M, N, K,
                                             NNZ, Ap,
                                             Ai, Ax, B, C);
      if (thread_nz == 2)
...
void csrspmm_seqreduce_nnzbalance(const int M, const int N, const int K, const int NNZ,
                                  const int* Ap, const int* Ai, const float* Ax,
                                  const float *B, float *C) {
