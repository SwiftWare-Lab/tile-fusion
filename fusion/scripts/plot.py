...
def plot_spmm_spmm(logs_folder, file_name, baseline_implementation, target_imp):
    ...
    seq_exe_time, separated_exe_time, target_exe_time = [], [], []
    ...
    for mat in mat_list:
        ...
        target = cur_mat[cur_mat['Implementation Name'] == target_imp]
        target_exe_time.append(take_median(target))
    ...
    fig, ax = plt.subplots()
    ax.scatter(x_vals, np.array(separated_exe_time) / np.array(target_exe_time), facecolors='none', edgecolors='b',
               marker='s')
    ...
    ax.set_ylabel('Speedup ' + target_imp + ' vs ' + baseline_implementation, fontsize=20, fontweight='bold')
    ...
    fig.savefig(os.path.join(plot_folder, file_name[:-4] + '-' + target_imp + '.pdf'), bbox_inches='tight')
    # fig.show()


def plot_spmm_spmm_for_logs(logs_folder, baseline_implementation, target_imp):
    with os.scandir(logs_folder) as entries:
        for entry in entries:
            print(entry.name)
            # if entry is csv file
            if entry.name.endswith(".csv") and entry.is_file():
                plot_spmm_spmm(logs_folder, entry.name, baseline_implementation, target_imp)


if __name__ == '__main__':
    plot_spmm_spmm_for_logs(sys.argv[1], sys.argv[2], sys.argv[3])
