...
def plot_gcn(log_folder, log_file_name, config):
    ...
    for i, bcol in enumerate(bcols):
        ...
        mat_list = list(df_fusion_sorted_bcol['Matrix Name'].unique())
        ...
        for mat in mat_list:
            ...
            mat_gflops.append((cur_mat_nnz * bcol + rows*bcol*bcol) * 2 / 1e9)
            for x in impls:
        ...
        for impl in impls:
            ...
            # ax.plot(plt_x, gflops[impl], color=impl_colors[impl], label=impl_representations[impl], linewidth='0.3')
    ...
    axs[0].set_ylabel('GFLOPs per second', fontweight='bold', fontsize=7.5)
