...
#ifndef FUSED_GCN_GRAPHCOLORING_H
#define FUSED_GCN_GRAPHCOLORING_H
...
public:
    std::map<int, std::vector<int>>
    generateGraphColoringForConflictGraphOf(int NumberOfNodes, int *IndPtr, int *RowIndices, int TileSize,
                                            int OutputSize, int KTileSize) {
        std::map<std::string, std::vector<std::string>> conflictGraph =
                createTilesConflictGraphWithKTiling(NumberOfNodes, IndPtr, RowIndices, TileSize, OutputSize,
                                                    KTileSize);
...
protected:
    std::map<std::string, std::vector<std::string>>
    createTilesConflictGraphWithKTiling(int NumberOfNodes, int *IndPtr, int *RowIndices, int TileSize,
                                        int OutputSize, int KTileSize) {
        std::map<std::string, std::vector<std::string>> conflictGraph;
        int numOfTiles = (int)ceil((double)NumberOfNodes / TileSize);
        int numOfKTiles = OutputSize / KTileSize;
        for (int i = 0; i < numOfTiles; i++) {
            int iStart = i * TileSize;
            int iEnd = std::min(iStart + TileSize, (int)NumberOfNodes);
            int aSize = IndPtr[iEnd] - IndPtr[iStart];
            int *a = new int[aSize];
            ...
            for (int j = i + 1; j < numOfTiles; j++) {
                int jStart = j * TileSize;
                int jEnd = std::min(jStart + TileSize, (int)NumberOfNodes);
                int bSize = IndPtr[jEnd] - IndPtr[jStart];
                int *b = new int[bSize];
                std::memcpy(a, RowIndices + IndPtr[iStart], aSize * sizeof(int));
                std::memcpy(b, RowIndices + IndPtr[jStart], bSize * sizeof(int));
                if (checkIfTwoArraysHasSameValue(a, b, aSize, bSize)) {
...
#endif //FUSED_GCN_GRAPHCOLORING_H
