...
class FusedGCNLayer(torch.nn.Module):
    ...
    def __init__(self, feat_dim, embed_dim, m_tile_size, adj, num_threads):
        ...
        if (feat_dim > embed_dim):
            self.schedule_data = torch.ops.sw_gcn.inspect_vt_ro(adj, feat_dim, embed_dim, 1000000, num_threads)
            self.forward_fn = torch.ops.sw_gcn.fusedGeMMSpMM_vt_ro
        else:
            self.schedule_data = torch.ops.sw_gcn.inspect_vt_ro(adj, embed_dim, feat_dim, 1000000, num_threads)
